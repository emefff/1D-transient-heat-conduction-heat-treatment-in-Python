...
t_heating = 4*3600   # heating time, s
...
step_size = t_heating/2000
# alpha = kw/(rhow*cpw)               # thermal diffusivity steel, m^2/s
t = np.arange(0, t_heating, step_size) # time range for simulation, s
...
# the h-values for radiation are calculated with help from https://www.engineeringtoolbox.com/radiation-heat-transfer-d_431.html
# input values were 0.9, 900, a_temperature, 1 and dividede by the temperature 
# we also add a constant value for convective HT in air
h_const = 30 # W/m²K for convection in still air
h_list_temperature = [20, 100, 200, 300, 400, 500, 600, 700, 800, 900]
# h_list_values_heating = [300 ,96121/25+h_const, 95626/100+h_const, 94060/200+h_const, \
#                           91113/300+h_const, 86145/400+h_const, 78393/500+h_const,\
#                           66972/600+h_const, 50874/700+h_const, 28967/800+h_const, \
#                           329/899+h_const] # for extreme value differences like here, we need a lot of split intervals!
h_list_values_heating = [500, 490, 480, 470, 460, 450, 440, 430, 422, 415]
...
num_time_intervals_heating = 100 # the larger the smoother the curves but can lead to bifurcating oscillations if too small
...
for i in tqdm.tqdm(range(num_time_intervals_heating)):
    ...
    t = np.arange(t_init, t_end, step_size) # time range for simulation, s there was a +0.002 in t_end
    ...
    T_avg = (Tr_slab[-1] + To_slab[-1]) / 2 - 273.15 # T_avg for next iteration, not pretty but ok for now
...
num_time_intervals_cooling = 100
...
for i in tqdm.tqdm(range(num_time_intervals_cooling)):
    ...
    # this is our new split time interval
    t = np.arange(t_init, t_end, step_size) # time range for simulation, s there was a +0.002 in t_end
    t_new = np.arange(t_init_new, t_end_new, step_size) # only for continuous time in final t-list, otherwise Fo is completely wrong!
...
Th = Th - 273.15
for i in range(2):
    # ylim[i] = ylim[i] - 273.15
    ylimRange[i] = ylimRange[i] - 273.15
...
polyorder = 3
deriv = 0
To_slab_list_filter = savgol_filter(To_slab_list, window_size, polyorder, deriv)
Tr_slab_list_filter = savgol_filter(Tr_slab_list, window_size, polyorder, deriv) 


# we plot the results for the slab
plt.figure(figsize=(15,9))
# plt.plot(t_list, To_slab_list, '-g', lw=2, label='surface')
plt.plot(t_list, To_slab_list_filter, '--g', lw=2, label='surface filtered')
# plt.plot(t_list, Tr_slab_list, '-r', lw=2, label='center')
